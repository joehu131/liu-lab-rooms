...
test('User Preferences Sanitation & Defaults', async (t) => {
  await t.test('returns exact default preferences when raw input is null, undefined, array, or non-object', () => {
    ...
    assert.deepEqual(sanitizePreferences(['nested']), DEFAULT_PREFERENCES);
  });
  ...
  await t.test('gracefully repairs corrupted or invalid keys and deduplicates buildings', () => {
    const corrupted = {
      theme: 'neon-pink', // invalid
      lang: 'fr', // invalid
      selectedOs: 'macOS', // invalid
      selectedBuildings: ['B-huset', 'B-huset', 123, null, 'All', 'NonExistentBuilding'], // deduplicates and filters
    ...
    assert.deepEqual(sanitized.selectedBuildings, ['B-huset']); // filtered and deduplicated
