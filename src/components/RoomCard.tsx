...
const RoomCardComponent: React.FC<RoomCardProps> = ({ availability, currentHour }) => {
  ...
  let statusText = 'Hela dagen';
  let fullExplanation = 'Ledig hela dagen';
  ...
  if (status === 'BUSY') {
    ...
    statusText = busyUntilStr ? busyUntilStr : 'Upptagen';
    fullExplanation = busyUntilStr ? `Upptagen till ${busyUntilStr}` : 'Upptagen';
    if (currentBooking?.courseCode) {
      fullExplanation += ` (${currentBooking.courseCode})`;
    }
    ...
    statusText = nextStartStr ? `${nextStartStr}${minsStr}` : 'Snart upptagen';
    fullExplanation = nextStartStr ? `Ledig till ${nextStartStr} (${formatMinutes(freeMinutesRemaining || 0)} kvar)` : 'Snart upptagen';
  } else if (status === 'FREE') {
    if (freeMinutesRemaining !== undefined && freeUntil) {
      ...
      fullExplanation = `Ledig till ${nextStartStr} (${formatMinutes(freeMinutesRemaining)} kvar)`;
    } else {
      statusText = 'Hela dagen';
      fullExplanation = 'Ledig hela dagen';
    }
  }
    ...
    <div className="room-row flex flex-col">
      ...
      >
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 min-w-0 flex-1 overflow-hidden">
          ...
          <span
            className={`inline-flex items-center gap-1 px-1 sm:px-1.5 py-0.2 rounded text-[9px] sm:text-[9.5px] font-mono font-medium border border-[var(--rule)] shrink-0 ${
              isLinux
          ...
          <span className="text-[10px] sm:text-[10.5px] text-[var(--ink-3)] font-mono truncate min-w-0">
            {room.building} • Plan {room.floor} • {room.computers} datorer
          </span>
        </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 pb-1.5 border-b border-[var(--rule-faint)]">
            <div className="flex items-center gap-3 sm:gap-4 flex-wrap text-[11px] text-[var(--ink-2)]">
              <div className="flex items-center gap-1.5">
                ...
                <span>{room.building}, Plan {room.floor}</span>
              </div>
              <div className="flex items-center gap-1.5">
            ...
            >
              ...
              <span>Öppna i Mazemap (Plan {room.floor})</span>
            </a>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            {todayBookings.length === 0 ? (
              <div className="text-[11px] text-status-free flex items-center gap-1.5 py-0.5">
                ...
                <span>Inga bokningar under dagen • Salen är ledig hela dagen</span>
              </div>
            ) : (
              <div className="space-y-1.5">
